/**
 * Acctventa commerce UI — cart drawer, wishlist, disputes, social proof, and chat-safety
 * banners layered on top of dashboard-app.js. Talks to the backend through window.AcctventaApi
 * (see api/commerce_features.php + api/index.php for the matching server actions).
 */
(function (global) {
  function Api() {
    return global.AcctventaApi;
  }
  function A() {
    return global.Acctventa;
  }
  ...
  function toast(msg, opts) {
    ...
    if (global.AcctventaToast && typeof global.AcctventaToast.show === 'function') {
      global.AcctventaToast.show(msg, opts || {});
    } else {
      global.alert(msg);
    }
  }
  ...
  async function checkoutCart() {
    ...
    try {
      ...
      closeCartDrawer();
      await refreshCartBadge();
      if (global.AcctventaApiSync) await global.AcctventaApiSync.hydrateFromApi();
  ...
  function handleBlockedChatMessage(reason) {
    if (global.AcctventaToast && typeof global.AcctventaToast.error === 'function') {
      global.AcctventaToast.error(reason || 'That message was blocked. Please keep all communication on Acctventa.');
    } else {
      global.alert(reason || 'That message was blocked. Please keep all communication on Acctventa.');
    }
  }
