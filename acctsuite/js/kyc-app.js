/**
 * AcctSuite Business KYC — multi-step business verification wizard.
 */
(function (global) {
  ...
  function toast(msg, type) {
    if (global.AcctSuiteToast) global.AcctSuiteToast.show(msg, { type: type || 'info' });
    else alert(msg);
  }
  ...
  async function refreshStatus() {
    ...
    try {
      ...
      if (global.AcctSuiteApi && (await global.AcctSuiteApi.isAvailable()) && global.AcctSuiteApi.getToken()) {
        const res = await global.AcctSuiteApi.kycStatus();
        statusCache = {
    ...
    if ((st.isVerified || st.kycStatus === 'verified') && name === 'intro') {
      el.innerHTML = `
        <div class="kyc-hero">
          ...
          <button type="button" class="kyc-btn-primary" onclick="window.AcctSuiteKyc.close()">Done</button>
        </div>`;
      return;
    }
    if (['needs_review', 'blurry_review', 'pending'].includes(st.kycStatus) && name === 'intro') {
      el.innerHTML = `
        <div class="kyc-hero">
          ...
          <button type="button" class="kyc-btn-primary" onclick="window.AcctSuiteKyc.close()">Close</button>
        </div>`;
      return;
    }
    ...
    if (name === 'intro') {
      el.innerHTML = `
        <div class="kyc-hero">
          ...
          <button type="button" class="kyc-btn-ghost" onclick="window.AcctSuiteKyc.close()">I'll do this later</button>
        </div>`;
    ...
    if (name === 'business') {
      el.innerHTML = `
        <div class="kyc-section">
          ...
          <p class="kyc-lead">Tell us about the business selling on AcctSuite.</p>
  ...
  async function submitKyc() {
    ...
    try {
      if (!global.AcctSuiteApi || !(await global.AcctSuiteApi.isAvailable())) {
      ...
      const res = await global.AcctSuiteApi.kycSubmit(payload);
      statusCache = {
        isVerified: !!res.isVerified,
      ...
      updateSidebarButton();
      if (res.user && global.AcctSuiteApi.applySessionUser) {
        global.AcctSuiteApi.applySessionUser(res.user);
      }
