 ...
 */
(function (global) {
  const KEY = 'acctsuite_staff_inapp_alerts';
  let titleTimer = null;
  let originalTitle = '';
  ...
  function ensureToastStyles() {
    if (document.getElementById('acctsuiteStaffAlertStyles')) return;
    const style = document.createElement('style');
    style.id = 'acctsuiteStaffAlertStyles';
    style.textContent =
      '#acctsuiteStaffToast{position:fixed;left:50%;bottom:24px;transform:translateX(-50%) translateY(120%);z-index:99999;max-width:min(92vw,380px);background:#0f172a;color:#fff;border:1px solid #F85830;border-radius:14px;padding:12px 14px;box-shadow:0 12px 40px rgba(0,0,0,.35);transition:transform .25s ease;font-family:system-ui,sans-serif}' +
      ...
      '#acctsuiteStaffNotifStatus{font-size:11px;opacity:.8}';
  ...
  function showToast(title, body) {
    if (global.AcctSuiteToast && typeof global.AcctSuiteToast.show === 'function') {
      ...
      global.AcctSuiteToast.show(msg || 'New message', {
        type: 'info',
        duration: 4500,
      });
      return;
    ...
    ensureToastStyles();
    let el = document.getElementById('acctsuiteStaffToast');
    if (!el) {
      el = document.createElement('div');
      el.id = 'acctsuiteStaffToast';
