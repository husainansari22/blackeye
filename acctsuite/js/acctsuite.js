/**
 * AcctSuite core — per-user marketplace data (localStorage).
 * Brand colors stay in CSS. No demo identity data.
 */
(function (global) {
  const USERS_KEY = 'acctsuite_users';
  const SESSION_KEY = 'acctsuite_session';
  const SETTINGS_KEY = 'acctsuite_settings';
  const ADMIN_KEY = 'acctsuite_admin';
  ...
  const DEFAULT_CONFIG = {
    ...
    siteName: 'acctsuite',
    siteTagline: 'Buy & sell digital accounts safely',
    announcementEnabled: false,
    announcementText: '',
    maintenanceMode: false,
    maintenanceMessage: '',
    listingsPaused: false,
    listingsPausedMessage: '',
  ...
  function applySettings(settings) {
    ...
    // expose live objects
    if (global.AcctSuite) {
      global.AcctSuite.CONFIG = CONFIG;
      global.AcctSuite.PLANS = PLANS;
      global.AcctSuite.GATEWAYS = GATEWAYS;
    }
  ...
  function adminLogin(username, password) {
    ...
    try {
      sessionStorage.setItem('acctsuite_admin_session', JSON.stringify({ at: Date.now(), user: admin.username }));
    } catch (e) {
      // still allow login for this page session
      global.__acctsuiteAdminOk = true;
    }
  ...
  function adminLogout() {
    try {
      sessionStorage.removeItem('acctsuite_admin_session');
    } catch (e) {}
    global.__acctsuiteAdminOk = false;
  }

  function isAdminLoggedIn() {
    if (global.__acctsuiteAdminOk) return true;
    try {
      const s = JSON.parse(sessionStorage.getItem('acctsuite_admin_session') || 'null');
      return !!(s && s.user);
    } catch (e) {
      return false;
    }
  ...
  // (boot continues after AcctSuite is assigned below)
  ...
  function allowedHostsForCategory(category) {
    ...
    if (window.AcctSuiteCatalog) {
      const hit = window.AcctSuiteCatalog.findProduct(cat);
      if (hit && hit.domain) return [hit.domain];
      const group = (window.AcctSuiteCatalog.GROUPS || []).find((g) => g.name === cat || g.id === cat);
  ...
  function createAd(user, draft) {
    ...
    try {
      if (global.AcctSuiteApi && global.AcctSuiteApiSync) {
        if (typeof global.AcctSuiteApiSync.patchAcctSuiteForApi === 'function') {
          global.AcctSuiteApiSync.patchAcctSuiteForApi();
        }
        if (global.AcctSuite && global.AcctSuite.__apiPatched && global.AcctSuite.createAd !== createAd) {
          return global.AcctSuite.createAd(user, draft);
        }
        return {
          ok: false,
    ...
    setTimeout(() => {
      ...
      if (typeof global.AcctSuiteUI !== 'undefined' && global.AcctSuiteUI.onAdsUpdated) {
    ...
    try {
      ...
      if (Api && typeof Api.getToken === 'function' && Api.getToken() && typeof Api.upgradePlan === 'function') {
        return Api.upgradePlan({ planId: String(planId), method: method === 'free' ? 'wallet' : method }).then(function (res) {
          ...
          if (global.AcctSuiteApiSync && typeof global.AcctSuiteApiSync.hydrateFromApi === 'function') {
            return global.AcctSuiteApiSync.hydrateFromApi().then(function () {
              return {
                ok: true,
  ...
  try {
    ...
    console.warn('AcctSuite boot warning:', e);
  }
})(window);
