...
try {
    switch ($action) {
        ...
        case 'auth.forgot': {
            ...
            $resetUrl = rtrim(app_config()['app_url'] ?? 'https://acctsuite.com', '/') . '/reset.html?token=' . urlencode($raw);
            $mail = email_password_reset($u['name'], $resetUrl);
            $sent = send_app_mail($email, $mail['subject'], $mail['html'], $mail['text']);
            if (!$sent) {
                json_out(['ok' => false, 'error' => 'Could not send email right now. Create mailbox support@acctsuite.com in Hostinger and try again.'], 500);
            }
            json_out([
                'ok' => true,
        ...
        case 'disputes.open': {
            ...
            notify_order_parties_email($o, 'Dispute opened', 'The buyer opened a dispute within the 60-minute window. Our team will review the order chat. Keep all communication on AcctSuite.');
        ...
        case 'plans.upgrade': {
            ...
            $checkout = flw_create_checkout($u, $price, $txRef, $prefer ?: 'NGN', [
                ...
                'title' => (app_config()['app_name'] ?? 'AcctSuite') . ' Plan',
                'description' => 'Upgrade to ' . ($plan['name'] ?? $planId) . ' — ' . (int)$plan['daily_uploads'] . ' uploads/day',
                'redirect_url' => rtrim((string)(app_config()['app_url'] ?? 'https://acctsuite.com'), '/') . '/wallet-return.html?purpose=plan',
        ...
        case 'cron.reconcile_payments': {
            ...
            if ($expected === '') {
                $expected = substr(hash('sha256', (string)(app_config()['owner_password'] ?? 'owner') . '|acctsuite-flw-cron'), 0, 32);
            }
        ...
        case 'staff.login': {
            ...
            $name = $okOwner ? 'Owner Support' : 'AcctSuite Support';
