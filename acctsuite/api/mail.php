    ...
    <tr>
      <td align="center">
        <table role="presentation" width="100%" style="max-width:560px;background:#111827;border:1px solid #1f2937;border-radius:20px;overflow:hidden;">
          <tr>
            <td style="padding:28px 28px 12px;background:linear-gradient(135deg,#04101C 0%,#111827 55%,#111827 100%);">
...
function email_button(string $label, string $href): string {
    ...
    return '<a href="' . $href . '" style="display:inline-block;background:' . $brand . ';color:#ffffff;text-decoration:none;font-weight:700;font-size:14px;padding:14px 28px;border-radius:12px;box-shadow:0 8px 20px rgba(10,39,72,0.25);">' . $label . '</a>';
}
...
function email_order_notice(string $name, string $title, string $role, string $amount, string $txid = '', string $releaseNote = ''): array {
    ...
    if ($role === 'seller') {
        ...
        $body = 'Great news! A buyer just purchased <strong style="color:#fff;">' . $safeTitle . '</strong> for <strong style="color:#0A2748;">$' . $safeAmount . '</strong>.';
        if ($releaseNote !== '') {
            $body .= ' ' . htmlspecialchars($releaseNote, ENT_QUOTES, 'UTF-8');
        }
        $subject = '🎉 Sale confirmed · ' . $title . ' · AcctSuite';
        $extra = '
      <div style="margin:18px 0;padding:16px;border-radius:14px;background:rgba(10,39,72,0.12);border:1px solid rgba(10,39,72,0.35);">
        <p style="margin:0 0 6px;font-size:12px;font-weight:800;color:#0A2748;text-transform:uppercase;letter-spacing:0.04em;">What to do next</p>
        ...
        $body = 'Your order for <strong style="color:#fff;">' . $safeTitle . '</strong> (<strong style="color:#0A2748;">$' . $safeAmount . '</strong>) is confirmed. Open <strong style="color:#fff;">My Purchase</strong> to view credentials and chat with the seller.';
        $subject = 'Purchase confirmed · ' . $title . ' · AcctSuite';
        $extra = '
      <div style="margin:18px 0;padding:16px;border-radius:14px;background:rgba(10,39,72,0.12);border:1px solid rgba(10,39,72,0.35);">
        <p style="margin:0 0 6px;font-size:12px;font-weight:800;color:#0A2748;text-transform:uppercase;letter-spacing:0.04em;">Buyer protection</p>
...
function email_order_status_update(string $name, string $title, string $statusLabel, string $txid = '', string $detail = ''): array {
    ...
    $inner = '
      ...
      <p style="margin:0 0 12px;font-size:14px;line-height:1.6;color:#cbd5e1;">Hi ' . $safeName . ', your order <strong style="color:#fff;">' . $safeTitle . '</strong> is now <strong style="color:#0A2748;">' . $safeStatus . '</strong>.</p>
