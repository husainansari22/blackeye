...
/**
 * AcctSuite branded HTML email templates + send helper (Hostinger mail()).
 */
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="100%" style="max-width:560px;background:#111827;border:1px solid #1f2937;border-radius:20px;overflow:hidden;">
          <tr>
            <td style="padding:28px 28px 12px;background:linear-gradient(135deg,#9A3412 0%,#111827 55%,#111827 100%);">
...
function email_button(string $label, string $href): string {
    ...
    return '<a href="' . $href . '" style="display:inline-block;background:' . $brand . ';color:#ffffff;text-decoration:none;font-weight:700;font-size:14px;padding:14px 28px;border-radius:12px;box-shadow:0 8px 20px rgba(248,88,48,0.25);">' . $label . '</a>';
}
...
function email_password_reset(string $name, string $resetUrl): array {
    ...
    return ['subject' => 'Reset your AcctSuite password', 'html' => $html, 'text' => $text];
}
...
function email_welcome(string $name): array {
    ...
    return ['subject' => 'Welcome to AcctSuite', 'html' => $html, 'text' => $text];
}
...
function email_password_changed(string $name): array {
    ...
    $inner = '
      ...
      <p style="margin:0 0 12px;font-size:14px;line-height:1.6;color:#cbd5e1;">Hi ' . $safeName . ', your AcctSuite password was changed successfully. You can sign in with your new password anytime.</p>
    ...
    return ['subject' => 'Your AcctSuite password was changed', 'html' => $html, 'text' => "Hi {$name}, your password was changed. If this wasn't you, contact support."];
}
...
function email_order_notice(string $name, string $title, string $role, string $amount, string $txid = '', string $releaseNote = ''): array {
    ...
    if ($role === 'seller') {
        ...
        $body = 'Great news! A buyer just purchased <strong style="color:#fff;">' . $safeTitle . '</strong> for <strong style="color:#F85830;">$' . $safeAmount . '</strong>.';
        if ($releaseNote !== '') {
            $body .= ' ' . htmlspecialchars($releaseNote, ENT_QUOTES, 'UTF-8');
        }
        $subject = '🎉 Sale confirmed · ' . $title . ' · AcctSuite';
        $extra = '
      <div style="margin:18px 0;padding:16px;border-radius:14px;background:rgba(248,88,48,0.12);border:1px solid rgba(248,88,48,0.35);">
        <p style="margin:0 0 6px;font-size:12px;font-weight:800;color:#F85830;text-transform:uppercase;letter-spacing:0.04em;">What to do next</p>
        ...
        $body = 'Your order for <strong style="color:#fff;">' . $safeTitle . '</strong> (<strong style="color:#F85830;">$' . $safeAmount . '</strong>) is confirmed. Open <strong style="color:#fff;">My Purchase</strong> to view credentials and chat with the seller.';
        $subject = 'Purchase confirmed · ' . $title . ' · AcctSuite';
        $extra = '
      <div style="margin:18px 0;padding:16px;border-radius:14px;background:rgba(248,88,48,0.12);border:1px solid rgba(248,88,48,0.35);">
        <p style="margin:0 0 6px;font-size:12px;font-weight:800;color:#F85830;text-transform:uppercase;letter-spacing:0.04em;">Buyer protection</p>
...
function email_order_status_update(string $name, string $title, string $statusLabel, string $txid = '', string $detail = ''): array {
    ...
    $inner = '
      ...
      <p style="margin:0 0 12px;font-size:14px;line-height:1.6;color:#cbd5e1;">Hi ' . $safeName . ', your order <strong style="color:#fff;">' . $safeTitle . '</strong> is now <strong style="color:#F85830;">' . $safeStatus . '</strong>.</p>
