...
function ai_blocks_external_contact(string $text): array {
    ...
    $linkPatterns = [
        '/\bwa\.me\//i' => 'Links to WhatsApp are not allowed here. Please keep all contact on AcctSuite.',
        '/\bapi\.whatsapp\.com\b/i' => 'Links to WhatsApp are not allowed here. Please keep all contact on AcctSuite.',
        '/\bt\.me\//i' => 'Links to Telegram are not allowed here. Please keep all contact on AcctSuite.',
        '/\btelegram\.me\//i' => 'Links to Telegram are not allowed here. Please keep all contact on AcctSuite.',
        '/\bsignal\.me\//i' => 'Links to Signal are not allowed here. Please keep all contact on AcctSuite.',
        '/\bdiscord\.gg\//i' => 'Discord invite links are not allowed here. Please keep all contact on AcctSuite.',
        '/\bm\.me\//i' => 'Messenger links are not allowed here. Please keep all contact on AcctSuite.',
    ];
    ...
    $shortInvites = [
        ...
        '/\btelegram\s+me\b/i',
        '/\bfind\s+me\s+on\b/i',
        '/\boutside\s+(the\s+|this\s+)?(platform|app|site|acctsuite)\b/i',
        '/\boff[\s-]?platform\b/i',
    ...
    foreach ($shortInvites as $re) {
        if (preg_match($re, $lower)) {
            return ['blocked' => true, 'reason' => 'Please keep all communication on AcctSuite. Invitations to chat elsewhere are not allowed.'];
        }
    }
    ...
    if ($mentionsApp && $hasHandleOrNumber) {
        return ['blocked' => true, 'reason' => 'Sharing a messaging app handle or number is not allowed. Keep communication on AcctSuite.'];
    }
    ...
    if (preg_match('/\b(my|add|follow|find)\s+(ig|insta|instagram|snap|snapchat|telegram|whatsapp|discord|twitter|x)\s*(is|:)?\s*@?[\w.]{2,30}/i', $lower)) {
        return ['blocked' => true, 'reason' => 'Sharing a social handle to contact you outside AcctSuite is not allowed.'];
    }
    ...
    if ($looksLikePhone && $contactVerb) {
        return ['blocked' => true, 'reason' => 'Sharing a phone number to contact you outside AcctSuite is not allowed.'];
    }
