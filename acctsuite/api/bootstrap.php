...
function tx_public_id(array $row): string {
    ...
    $seed = hash('sha256', 'acctsuite-tx-' . (string)($row['id'] ?? '0') . '-' . (string)($row['created_at'] ?? ''));
...
function bearer_token(): ?string {
    ...
    if ($alt !== '') return $alt;
    if (!empty($_COOKIE['acctsuite_token'])) return (string)$_COOKIE['acctsuite_token'];
    return null;
}
...
function migrate_legacy_support_email(): void {
    ...
    try {
        ...
        if ($email === '' || $email === 'help@acctsuite.com') {
            setting_set('support_email', 'support@acctsuite.com');
        }
    } catch (Throwable $e) {
