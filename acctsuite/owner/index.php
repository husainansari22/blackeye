...
if ($authed && ($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    try {
        ...
        if ($form === 'gateway') {
            $webhookDefault = rtrim((string)(app_config()['app_url'] ?? 'https://acctsuite.com'), '/') . '/api/index.php?action=webhook.flutterwave';
        ...
        if ($form === 'verify_user') {
            ...
            if ($verified) {
                notify_user($uid, 'Account verified', 'Your AcctSuite profile now shows a verified badge. Buyers will see it on your storefront and listings.', 'kyc');
...
<head>
  ...
  <title>Owner Admin — AcctSuite</title>
...
<link rel="stylesheet" href="/vendor/fontawesome/css/all.min.css?v=20260905violet1">
  ...
  <div class="min-h-screen flex items-center justify-center p-4">
    <form method="post" class="w-full max-w-sm av-card p-6 space-y-4">
      ...
      <div class="text-center">
        <img src="/img/brand/logo-mark.svg?v=20260905violet1" alt="" class="w-12 h-12 mx-auto rounded-xl mb-2 shadow" width="48" height="48">
...
?>
  <header class="av-topbar">
    <div class="av-topbar-inner">
      <div class="av-brand"><img src="/img/brand/logo-mark.svg?v=20260905violet1" alt="" class="av-brand-mark" width="36" height="36" decoding="async"><span class="title truncate">Owner Admin</span></div>
  ...
  <main class="av-shell space-y-4 py-4<?= in_array($tab, ['support', 'chats', 'wallet'], true) ? ' av-shell-wide' : '' ?>">
      ...
      <div class="av-page av-settings-page">
        ...
        <div class="av-settings-group">
          <a class="av-settings-row" href="?tab=wallet">
            <span class="av-settings-icon" style="background:#8B5CF6"><i class="fa-solid fa-money-bill-transfer"></i></span>
        ...
        <div class="av-settings-group">
          <a class="av-settings-row" href="?tab=users">
            <span class="av-settings-icon" style="background:#8B5CF6"><i class="fa-solid fa-users"></i></span>
          ...
          <a class="av-settings-row" href="?tab=orders">
            <span class="av-settings-icon" style="background:#8B5CF6"><i class="fa-solid fa-bag-shopping"></i></span>
        ...
        <div class="av-settings-group">
          ...
          <a class="av-settings-row" href="?tab=plans">
            <span class="av-settings-icon" style="background:#A78BFA"><i class="fa-solid fa-crown"></i></span>
    ...
    ?>
      ...
      <script>
        ...
        const Inbox = () => window.AcctSuiteStaffInbox;
        ...
        function ownerEnableNotif(){
          if (window.AcctSuiteStaffAlerts) window.AcctSuiteStaffAlerts.enable({ buttonId: 'staffNotifBtn' });
          else alert('Alert helper failed to load. Hard-refresh and try again.');
        }
        function ownerNotify(t,b){
          if (window.AcctSuiteStaffAlerts) window.AcctSuiteStaffAlerts.notify(t, b);
        }
        ...
        if (window.AcctSuiteStaffAlerts) {
          window.AcctSuiteStaffAlerts.enable({ silent: true, buttonId: 'staffNotifBtn' });
          window.AcctSuiteStaffAlerts.updateButton('staffNotifBtn');
        }
      </script>
    <?php endif; ?>
      ...
      <div class="av-page av-settings-page">
        ...
        <div class="av-settings-group">
          <div class="av-settings-row is-static">
            <span class="av-settings-icon" style="background:#8B5CF6"><i class="fa-solid fa-wallet"></i></span>
        ...
        <div class="av-settings-group">
          ...
          <form method="post" action="?tab=users&amp;id=<?= (int)$u['id'] ?>" target="_blank" class="av-settings-row-form">
            ...
            <button type="submit" class="av-settings-row av-settings-row-btn" <?= $banned ? 'disabled' : '' ?>>
              <span class="av-settings-icon" style="background:#8B5CF6"><i class="fa-solid fa-right-to-bracket"></i></span>
                ...
                <div class="av-ad-line-body" hidden>
                  <div class="av-ad-detail-grid">
                    ...
                    <?php if ($soldOut && $st === 'active'): ?>
                      <div class="av-ad-detail-span2" style="color:#8B5CF6">Sold out — not shown on Home/Market until restocked.</div>
                    <?php endif; ?>
                  </div>
      ...
      <script>
        async function ownerSearchOrder(){
          ...
          try{
            ...
            const res=await fetch(url,{headers:{'Authorization':'Bearer '+(localStorage.getItem('acctsuite_staff_token')||''),'X-Staff-Token':(localStorage.getItem('acctsuite_staff_token')||'')}});
            const data=await res.json();
    ...
    ?>
      ...
      <script>
        ...
        async function apiStaff(action, opts={}){
          ...
          url.searchParams.set('action',action);
          if(opts.query) Object.entries(opts.query).forEach(([k,v])=>url.searchParams.set(k,v));
          const tok=localStorage.getItem('acctsuite_staff_token')||'';
        ...
        async function openOrderChat(id){
          ...
          document.getElementById('orderChatRefundBtn').onclick=async()=>{
            if(!(await AcctSuiteConfirm({title:'Refund order',message:'Refund buyer and deduct seller (negative OK)?',okText:'Refund buyer',icon:'fa-rotate-left',danger:true})))return;
          ...
          document.getElementById('orderWarrantyRefundBtn').onclick=async()=>{
            if(!(await AcctSuiteConfirm({title:'Warranty refund',message:'24h warranty refund: deduct seller (incl. commission clawback) and refund buyer full price?',okText:'Process refund',icon:'fa-shield-halved',danger:true})))return;
            try{
          ...
          (async()=>{
            ...
            try{
              ...
              document.getElementById('orderDisputeRefundBtn').onclick=async()=>{
                if(!(await AcctSuiteConfirm({title:'Resolve dispute',message:'Resolve dispute with refund to buyer?',okText:'Refund buyer',icon:'fa-gavel',danger:true})))return;
                try{
              ...
              document.getElementById('orderDisputeDenyBtn').onclick=async()=>{
                if(!(await AcctSuiteConfirm({title:'Deny dispute',message:'Deny this dispute?',okText:'Deny dispute',icon:'fa-ban',danger:true})))return;
                try{
                  ...
                  <div class="av-form-grid cols-2">
                    ...
                    ?>
                      <div class="av-field-block">
                        ...
                        <input name="crypto[<?= $i ?>][addr][<?= h($nk) ?>]" value="<?= h($addrVal) ?>" placeholder="Paste <?= h($nk) ?> wallet address" class="font-mono text-xs" style="<?= $addrVal === '' ? 'border-color:#8B5CF6' : '' ?>">
                        <?php if ($addrVal === ''): ?>
          ...
          <div class="av-panel-body space-y-4">
            <div class="av-admin-card">
              ...
              <div class="av-form-grid space-y-2">
                ...
                <div class="av-field-block"><label>Webhook</label><input name="deposit_webhook" value="<?= h(($gw['deposit_webhook']??'') !== '' ? $gw['deposit_webhook'] : 'https://acctsuite.com/api/index.php?action=webhook.flutterwave') ?>" placeholder="https://acctsuite.com/api/index.php?action=webhook.flutterwave"></div>
...
function toggleOwnerTheme(){
  ...
  try{localStorage.setItem('acctsuite_owner_theme', dark?'dark':'light');}catch(e){}
